...
señal_ecg_centrada = -(señal_ecg_promediada - offset)  # Centrar la señal alrededor de 0 e invertir
...
tiempo_continuo = np.linspace(0, 2 * 1750, len(señal_ecg_continua))  
...
plt.plot(tiempo_continuo, señal_ecg_continua, color='blue')
...
plt.figure(figsize=(12, 6))
plt.plot(tiempo_continuo, señal_filtrada_pasa_bajo, color='red')
...
plt.figure(figsize=(12, 6))
plt.imshow(coeficientes, extent=[0, len(señal_filtrada_pasa_bajo), scales.min(), scales.max()], cmap='PRGn', aspect='auto')
...
plt.figure(figsize=(12, 6))
plt.imshow(np.abs(coef_baja), extent=[0, len(intervalos_rr), 0.04, 0.15], aspect='auto', cmap='jet')
...
plt.figure(figsize=(12, 6))
plt.imshow(np.abs(coef_alta), extent=[0, len(intervalos_rr), 0.15, 0.4], aspect='auto', cmap='jet')
