...
Cuando estaba finalmente la señal en python, lo que se hizo fue agregar un Filtro pasa alto: Este filtro elimina las componentes de baja frecuencia (cutoff de 0.5 Hz), lo cual es útil para remover variaciones lentas o ruido de baja frecuencia, como el movimiento corporal o la variación de la línea base de la señal ECG. Este tipo de ruido puede oscurecer los detalles finos de la señal, como los picos R, que son necesarios para calcular los intervalos R-R en la variabilidad de la frecuencia cardíaca (HRV).
y uno  pasa bajo: Después del filtro pasa alto, se aplica un filtro pasa bajo con un cutoff de 30 Hz. Esto elimina las frecuencias altas, incluyendo el ruido de alta frecuencia generado por interferencias electromagnéticas y la actividad muscular, que pueden distorsionar la señal ECG. Este filtrado permite resaltar las frecuencias que son más características del ECG, preservando sus componentes importantes (por ejemplo, el complejo QRS) y asegurando que el análisis de la señal sea más preciso y confiable.
selecciónamos estos filtros en lugar de otros porque equilibran bien la eliminación de ruido sin comprometer la forma de onda de ECG esencial para el análisis, particularmente para el cálculo preciso de los picos R y la variabilidad de la frecuencia cardíaca (HRV).
...
En nuestro codigo la variación de las frecuencias a lo largo del tiempo se analiza mediante la Transformada Wavelet Continua (CWT) aplicada a los intervalos R-R de la señal ECG filtrada. Este análisis permite observar cómo las bandas de baja frecuencia (0.04 - 0.15 Hz) y alta frecuencia (0.15 - 0.4 Hz) cambian en intensidad y distribución temporal, reflejando la actividad simpática y parasimpática del sistema nervioso. Los espectrogramas muestran las variaciones en la potencia espectral en cada banda: en la banda de baja frecuencia, que está relacionada con la actividad simpática, los cambios en la potencia pueden señalar respuestas a factores como el estrés o cambios en la posición corporal, mientras que en la banda de alta frecuencia, asociada con la actividad parasimpática y la respiración, las variaciones pueden reflejar modificaciones en la frecuencia respiratoria. Así, el análisis revela no solo la presencia de estas frecuencias sino también cómo su potencia fluctúa, proporcionando información dinámica sobre el balance autonómico del sistema cardiovascular.
